...
import { SalesTransferModel } from '../src/models/salesTransferModel';
import { ReEnrollVeipEsppModel } from '../src/models/reEnrollVeipEsppModel';
import { GidSsnModel } from '../src/models/gidSsnModel';
import { LoggingModel } from '../src/models/loggingModel';
import { GrantCodeModel } from '../src/models/grantCodeModel';
import { TermCancellationModel } from '../src/models/processTermCancellationModel';
import { EAAccessModel } from '../src/models/eaAccessModel'
import { EsoAuthorizer } from '../authorizer';
import { OfficerCodeModel } from '../src/models/officerCodeModel';

var jwt = require('jsonwebtoken');

const myServices =  new FileService();
const auth =  new EsoAuthorizer();
...
async function initializeVariables () {
  const [accessKeyId, secretAccessKey] = await Promise.all([decryptor(process.env.accessKeyId), decryptor(process.env.secretAccessKey)]);
...
const blockedDate = new RegExp(/[?,!,+,&,$,;,',‘,%,[,"]/gmi);
  ...
  export async function getSpecificGrantHandler (event: any, context: any, callback: (...args: any[]) => void) : Promise <any> {
  //export function getSpecificGrantHandler (event: any, context: any, callback: (...args: any[]) => void) : void {  
    ...
    if(authResult['authResult'] == 'Authorized'){  
      ...
      if(event.body.actionType == 1){     
          return new Promise(resolve => {
            let myList = new LoggingModel(event.body.actionId, event.body.actionDetails, event.body.enterpriseId);
      ...
      else if (event.body.actionType == 0){
        return new Promise(resolve => {
            myServices.getSpecificGrant().then((result) => {
              let response = {
                  contentType: "application/json",
                  statusCode: 200,
                  body: JSON.stringify({            
  ...
  //------------------------------------------------------- SALES TRANSFER DELETION -------------------------------------------------------//

  export async function processSalesTransferDeletionHandler (event: any, context: any, callback: (...args: any[]) => void): Promise <any> {
  ...
  if(authResult['authResult'] == 'Authorized'){
    ...
    //1 - delete
      if(event.body.actionType == 0){
        return new Promise(resolve => {
          let myList = new SalesTransferModel(event.body.personnelNumber, "","", event.body.transactionDate);
          let PersonnelNbrChecker = myList.PersonnelNumber;
          let TransacDtChecker = myList.TransactionDate;
          let checker: boolean = blockedChars.exec(PersonnelNbrChecker) != null || blockedDate.exec(TransacDtChecker) != null
            if (checker != true) {
              myServices.getGrantList(myList).then((result) => {
                let response = {
                    contentType: "application/json",
                    statusCode: 200,
                    body: JSON.stringify({            
      ...
      if(event.body.actionType == 1){
        return new Promise(resolve => {
          let myList = new SalesTransferModel(event.body.personnelNumber, event.body.sqlCommand1, event.body.sqlCommand2, event.body.transDate);
          let PersonnelNbrChecker = myList.PersonnelNumber;
          let TransacDtChecker = myList.TransactionDate;
          let checker: boolean = blockedChars.exec(PersonnelNbrChecker) != null || blockedDate.exec(TransacDtChecker) != null
            if (checker != true) {
              myServices.processSalesTransferDeletion(myList).then((result) => {
                let response = {
                    contentType: "application/json",
                    statusCode: 200,
                    body: JSON.stringify({            
